...
/*   Updated: 2020/01/31 22:16:50 by jrouchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void print_fourmis(t_solution *solution, t_fourmis *fourmis)
{
	if (fourmis->path_index == -1)
		return ;
	ft_printf("L%u-%s", fourmis->name, t_ptr_room_list_at(t_path_list_get(&(solution->paths), fourmis->path_index), fourmis->room_index)->name);
}
